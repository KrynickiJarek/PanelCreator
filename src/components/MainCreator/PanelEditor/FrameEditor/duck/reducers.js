...
const frameReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case types.CHANGE_FRAME_FONT_WEIGHT:
      return {
    ...
    case types.OVER_FRAME_ALL:
      const copyFrameHoldersForAll = state.frameHolders;
      copyFrameHoldersForAll.forEach((element) => {
        if (element.type === "single") {
          element.framePrint.forEach(singleElement => {
            if (singleElement !== 0) {
              singleElement.over = action.item
            }
          })
        } else if (element.type === "multi") {
          element.framePrint.over = action.item
        }
      })
      return {
        ...state, frameHolders: copyFrameHoldersForAll, overFrameRender: !state.overFrameRender
      }
    ...
    case types.SET_FRONT_END_REDUCER_FRAME:
      return action.item
    default:
      return state
  }
